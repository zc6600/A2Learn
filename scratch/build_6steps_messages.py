        ...
        "updateComponents": {
            ...
            "components": [
                ...
                },
                {
                    ...
                    "content": "为了将“用计算代替比对”的 Mindset 落地到工程实践中，数据结构引入了两个核心基础设施：\n\n1. **<dfn title=\"将任意长度输入 Key 映射转换为固定范围无符号整数的数学算法\">哈希函数 (Hash Function)</dfn>**：负责接收任意格式的 Key（如字符串、身份证号、对象引用），通过位运算与散列算法计算出一个无符号整数。比如 `hash('Alice') = 154829`。\n2. **<dfn title=\"在物理内存中连续开辟的存储槽位数组\">槽位数组 (Bucket Array / Hash Table)</dfn>**：开辟容量为 `capacity` 的物理数组，将算出的哈希值对容量取模，得到物理下标：`index = hash(key) % capacity`。\n\n**常数时间复杂度 O(1) 的物理保障**：\n无论数组里当前保存的是 10 条数据还是 1,000,000 条数据，只要哈希函数的计算过程极快（只需几条 CPU 执行指令），定位特定 Key 所在内存地址的总耗时都是固定的！这就是 **<dfn title=\"执行耗时与数据总量规模完全无关的极致性能界限\">常数时间复杂度 O(1)</dfn>** 的物理来源。"
                },
                {
                    "id": "step4-sandbox",
                    ...
                    "language": "javascript",
                    "code": """// 演示 1：在有序数组 [10, 20, 30, 50, 60] 中插入数字 25
        ...
        "updateComponents": {
            ...
            "components": [
                ...
                },
                {
                    ...
                    "icon": "⚙️",
                    "estimatedReadTime": "4 分钟深度解析",
                    "content": "工程实现中对碰撞处理形成了两大主流解法：\n\n1. **<dfn title=\"每个槽位维护一条单链表，冲突节点追加到链表末尾\">链地址法 (Separate Chaining)</dfn>**：Java HashMap 采用此法。每个数组槽位指向一条单向链表。发生碰撞时将新节点追加到链表末尾。优点是实现简单、高容错；缺点是链表节点在物理内存中随机分布，<dfn title=\"CPU预取连续内存数据的硬件加速机制\">CPU 缓存局部性</dfn> 较差，存在指针内存开销。\n2. **<dfn title=\"不建链表，冲突时按线性或二次探测规则检查数组相邻闲置槽位\">开放寻址法 (Open Addressing)</dfn>**：Python dict 采用此法。所有数据都紧密平铺在连续数组空间里。当发生碰撞时，按探查规则（如 `(index + 1) % capacity`）寻找下一个空闲槽位。优点是数据物理连续，能极佳地利用 CPU Cache Line 硬件预取加速；缺点是接近满载时碰撞率剧增。\n\n> 💡 工业权衡：Java HashMap 选链地址法求高容错；Python dict 选开放寻址法求极致物理内存与 CPU 缓存性能。"
                },
                {
                    "id": "step5-summary-2",
                    "component": "AnalogyCard",
        ...
        "updateComponents": {
            ...
            "components": [
                ...
                },
                {
                    ...
                    "pillarsTitle": "核心原则",
                    "pillars": [
                        {"title": "拥挤度第一性原理", "description": "闲置槽位越少，冲突概率呈指数级上升，必须动态维持安全水位。", "icon": "📊"},
        ...
        "updateComponents": {
            ...
            "components": [
                ...
                },
                {
                    "id": "step4-implementation",
                    "component": "DetailedExplanation",
                    "title": "Hash Function & Bucket Array Engineering",
                    "icon": "⚙️",
                    "estimatedReadTime": "4 min read",
                    "content": "To implement the 'Computation over Comparison' mindset into code, engineering introduces two core components:\n\n1. **<dfn title=\"Mathematical algorithm converting arbitrary length Key into fixed unsigned integer\">Hash Function</dfn>**: Takes arbitrary Key input (string, ID, reference) and computes an integer index. E.g. `hash('Alice') = 154829`.\n2. **<dfn title=\"Contiguous memory array holding actual data values\">Bucket Array (Hash Table)</dfn>**: Allocates an array of size `capacity`. Modulo arithmetic maps integer hash to array index: `index = hash(key) % capacity`.\n\n**O(1) Hardware Guarantee**:\nWhether the hash table contains 10 items or 1,000,000 items, as long as the hash calculation executes in a few CPU instructions, lookup time is constant! This is the physical foundation of **<dfn title=\"Constant time performance independent of dataset size\">O(1) Complexity</dfn>**."
                },
                {
                    "id": "step4-sandbox",
                    "component": "InteractiveSandbox",
        ...
        "updateComponents": {
            ...
            "components": [
                ...
                },
                {
                    "id": "step4-implementation-2",
                    "component": "DetailedExplanation",
                    "title": "Separate Chaining vs Open Addressing & CPU Cache",
                    "icon": "⚙️",
                    "estimatedReadTime": "4 min read",
                    "content": "Engineering has converged on two dominant collision resolution strategies:\n\n1. **<dfn title=\"Each bucket maintains a singly linked list appending colliding nodes\">Separate Chaining</dfn>**: Used by Java HashMap. Each array slot holds a linked list. Collisions append to the list. Simple and resilient, but nodes are scattered in memory with poor <dfn title=\"Hardware acceleration prefetching contiguous memory into cache\">CPU Cache Locality</dfn>.\n2. **<dfn title=\"No linked lists; probes neighboring array slots on collision\">Open Addressing</dfn>**: Used by Python dict. All data elements lie contiguously in the array. On collision, it probes next available slot `(idx + 1) % capacity`. Excellent CPU Cache Line prefetching, but performance drops sharply when full.\n\n> 💡 Engineering Tradeoff: Java uses Chaining for resilience; Python uses Open Addressing for raw CPU cache performance."
                },
                {
                    "id": "step5-summary-2",
                    "component": "AnalogyCard",
