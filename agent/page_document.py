...
def _data_changes(before: Any, after: Any, path: str = "") -> list[tuple[str, Any, bool]]:
    """Return (json-pointer path, value, should_delete) data operations.

    Objects are diffed by key. Arrays and scalar values are atomic; replacing an
    array is intentional because list item identity belongs in PageDocument
    components, not in an A2UI data-model patch.
    """
...
class A2uiCompiler:
    ...
    def compile(self, previous: PageDocument | None, current: PageDocument) -> SyncPlan:
        ...
        # A2UI v0.9 cannot delete a single component. A reset avoids retaining
        # stale components, including unreachable children, in the client model.
        if removed_ids:
